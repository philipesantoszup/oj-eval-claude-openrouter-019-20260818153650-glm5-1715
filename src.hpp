...
// Optimized attention - process row by row from HBM to minimize SRAM
void Calculate(std::vector<Matrix *> keys, std::vector<Matrix *> values,
  ...
  for (size_t i = 0; i < keys.size(); ++i) {
    ...
    // Move K and V to SRAM (keep Q in HBM, fetch rows as needed)
    ...
    for (size_t r = 0; r < n; ++r) {
      // Fetch row r of Q from HBM (slower but uses less SRAM)
      Matrix* Q_r = matrix_memory_allocator.Allocate("Qr");
      gpu_sim.GetRow(current_query, r, Q_r, Position::kInGpuHbm);
      gpu_sim.MoveMatrixToSharedMem(Q_r);

      // Compute scores: Q_r * K^T
      ...
      // Softmax
      Matrix* soft = matrix_memory_allocator.Allocate("sf");
